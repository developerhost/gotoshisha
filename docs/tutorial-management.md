...
### 実装ファイル
- **ユーティリティ**: `app/utils/tutorial/storage.ts`
...
// チュートリアル状態をリセット
AsyncStorage.removeItem('tutorial_completed').then(() => {
  console.log('チュートリアルをリセットしました');
});
```
...
import { isTutorialCompleted, resetTutorialStatus, setTutorialCompleted } from '../utils/tutorial/storage';
...
### チュートリアルが表示されない
1. AsyncStorage の値を確認: `tutorial_completed` が `"true"` になっていないか
2. 認証状態を確認: ログイン完了後に表示される
3. コンソールログでエラーをチェック

### チュートリアルが毎回表示される
1. `setTutorialCompleted()` が正常に呼ばれているか確認
2. AsyncStorage の書き込み権限を確認
3. ネイティブ環境での AsyncStorage の動作を確認

### リセットが効かない
1. アプリの完全再起動を試す
2. AsyncStorage の値が実際に削除されているか確認
3. キャッシュクリアを試す
...
- `app/utils/tutorial/storage.ts` - チュートリアル状態管理
